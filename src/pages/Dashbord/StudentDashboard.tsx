...
import { selectRequests, selectFetchLoading } from "../../store/selectors/requestsSelectors";
...
console.log(user);
  ...
  const stats = useMemo(() => {
    ...
    const inReview = requests.filter((req) => req.status === ("In Review" as any)).length;
    ...
    () => [
      {
        ...
        render: (row) => (
          <div>
            <p className="font-semibold text-gray-900">{row.service?.name || "Unknown Service"}</p>
      ...
      },
      {
        key: "category",
        label: "CATEGORY",
        render: (row) => <span className="text-gray-700">{row.service?.name || "General"}</span>,
      },
      {
        key: "date",
        label: "DATE",
        render: (row) => (
          <span className="text-gray-600">{new Date(row.createdAt).toLocaleDateString()}</span>
        ),
      },
      {
        key: "status",
        ...
        render: (row) => {
          ...
          const isSuccess = row.status === "Approved" || row.status === "Success";
  ...
  return (
    <div className="flex min-h-screen bg-[#F4F7FB]">
      <main className="flex-1 p-4 md:p-6 overflow-y-auto">
        <Topbar title="Student Dashboard" showSearch={false} subtitle={`Welcome back, ${user?.name}! Here are your recent requests.`} />

        

        {/* Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
          <Card title="ACTIVE REQUESTS" value={isLoading ? "..." : stats.total.toString()}>
          ...
          <div className="p-4 rounded-xl text-white bg-gradient-to-r from-blue-900 to-blue-600 shadow-sm flex flex-col justify-center">
            <p className="text-sm font-medium text-blue-100">Tuition Balance Due</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
          ...
          <div className="relative h-[160px] rounded-xl overflow-hidden shadow-sm border border-gray-100 bg-[url('https://maps.gstatic.com/tactile/basepage/pegman_sherlock.png')] bg-cover bg-center group">
            ...
            <div className="absolute bottom-3 left-3 text-white">
              <p className="font-bold drop-shadow-md">International Student Office</p>
              <p className="text-xs font-medium drop-shadow-md mt-0.5">Building C, Room 204</p>
            </div>
          </div>
        </div>
      </main>
